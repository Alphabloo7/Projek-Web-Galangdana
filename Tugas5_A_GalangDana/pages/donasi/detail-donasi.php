<?php
include '../../koneksi.php';
require_once '../auth/keamanan.php';
include '../../navbar.php';
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    // Ambil target donasi berdasarkan bentuk
    if ($data['bentuk_donasi'] === 'uang') {
      $res_uang = mysqli_query($conn, "SELECT target_uang FROM donasi_uang WHERE id_donasi = '$id'");
      $uang = mysqli_fetch_assoc($res_uang);
      $data['target_donasi'] = $uang['target_uang'] ?? 0;
    } elseif ($data['bentuk_donasi'] === 'barang') {
      $res_barang = mysqli_query($conn, "SELECT target_paket, harga_paket FROM donasi_barang WHERE id_donasi = '$id'");
      $barang = mysqli_fetch_assoc($res_barang);
      $data['target_donasi'] = isset($barang['target_paket'], $barang['harga_paket'])
        ? $barang['target_paket'] * $barang['harga_paket']
        : 0;
    } else {
      $data['target_donasi'] = 0;
    }
  } else {
...
<head>
  ...
  <title><?= $data ? htmlspecialchars($data['judul_donasi']) : 'Donasi Tidak Ditemukan' ?> - Donasi</title>
...
<body>
        ...
        <div class="organization-card">
          <div class="d-flex align-items-center">
            <div class="fs-5 fw-bold text-danger">
              <i class="fas fa-coins me-2"></i>Target Donasi: Rp <?= number_format($data['target_uang'] ?? 0, 0, ',', '.') ?>
            </div>
          </div>
        </div>
        ...
        <div class="donation-content mt-4">
          <form action="proses_transaksi.php" method="POST" onsubmit="return validateDonation();">
            ...
            <div class="mb-4">
              ...
              <p><strong>Bentuk Donasi:</strong> <?= htmlspecialchars($data['bentuk_donasi']) ?></p>
            </div>
  ...
  <script>
    ...
    function tampilkanNotifikasi(pesan, isError) {
      ...
      notif.className = 'alert alert-dismissible fade show position-fixed w-100';
      notif.classList.add(isError ? 'alert-danger' : 'alert-success');
      setTimeout(() => {
